...
class WallTrackerRosPublisher:
    """Optional ROS bridge for standalone wall_tracker.py.

    Publishes Float64MultiArray on a single topic so this ops tool can stream
    pose without becoming a ROS package or requiring custom message generation.
    """
    ...
    def __init__(self, topic: str):
        ...
        self.node = rclpy.create_node("wall_tracker")
        self.pub = self.node.create_publisher(Float64MultiArray, topic, 10)
        self._prev_t: Optional[float] = None
        self._prev_rel: Optional[np.ndarray] = None
        self._prev_vel = np.zeros(3, dtype=np.float64)
        self._vel_alpha = 0.55
        self.node.get_logger().info(
            f"Publishing wall tracker pose on {topic}: {','.join(self.FIELDS)}"
        )

    def publish(self, result: FrameResult) -> None:
        msg = self._Float64MultiArray()
        dim = self._MultiArrayDimension()
        dim.label = ",".join(self.FIELDS)
        dim.size = len(self.FIELDS)
        dim.stride = len(self.FIELDS)
        msg.layout.dim.append(dim)
        msg.layout.data_offset = 0

        status_code = self.STATUS_CODES.get(result.status, 0.0)
        ...
        p = result.pose
        abs_t = np.asarray(p.operator_abs_smooth, dtype=np.float64).reshape(3)
        rel_t = np.asarray(p.operator_t_smooth, dtype=np.float64).reshape(3)
        vel = self._relative_velocity(float(result.timestamp), rel_t)
        angles = np.asarray(p.rel_euler_smooth_deg, dtype=np.float64).reshape(3)
        valid = 1.0 if result.status in ("GOOD", "WARN", "HOLD") else 0.0
        msg.data = [
            float(result.timestamp),
            status_code,
            valid,
            float(abs_t[0]), float(abs_t[1]), float(abs_t[2]),
            float(rel_t[0]), float(rel_t[1]), float(rel_t[2]),
            float(vel[0]), float(vel[1]), float(vel[2]),
            float(angles[0]), float(angles[1]), float(angles[2]),
            float(p.omega_rad_s),
            float(p.omega_deg_s),
            float(p.depth_m),
            float(p.reproj_error_px),
            float(p.center_err_mm),
            float(len(p.tag_ids)),
        ]
        self.pub.publish(msg)
        self.spin_once()

    def _relative_velocity(self, timestamp: float, rel_t: np.ndarray) -> np.ndarray:
        if self._prev_t is None or self._prev_rel is None:
            self._prev_t = timestamp
            self._prev_rel = rel_t.copy()
            return self._prev_vel.copy()
        dt = timestamp - self._prev_t
        if dt <= 1.0e-4:
            return self._prev_vel.copy()
        raw = (rel_t - self._prev_rel) / dt
        self._prev_vel = self._vel_alpha * raw + (1.0 - self._vel_alpha) * self._prev_vel
        self._prev_t = timestamp
        self._prev_rel = rel_t.copy()
        return self._prev_vel.copy()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ros", action="store_true",
                        help="Publish position/velocity as Float64MultiArray")
    parser.add_argument("--ros-topic", default="/wall_tracker/pose",
                        help="Topic used when --ros is enabled")
    return parser
...
def main() -> int:
    ...
    ros_pub = WallTrackerRosPublisher(args.ros_topic) if args.ros else None
            ...
            while pipeline.isRunning():
                ...
                while True:
                    ...
                    if ros_pub is not None:
                        ros_pub.publish(latest_display_result)
