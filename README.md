...
With `--ros`, the tracker publishes `std_msgs/Float64MultiArray` on
`/wall_tracker/pose` by default. The array layout label names the fields:

```text
time_sec,status_code,valid,
x_abs_m,y_abs_m,z_abs_m,
dx_m,dy_m,dz_m,
vx_rel_m_s,vy_rel_m_s,vz_rel_m_s,
roll_deg,pitch_deg,yaw_deg,
omega_rad_s,omega_deg_s,
depth_m,reproj_error_px,center_err_mm,tag_count
```

Check it from another terminal:

```bash
ros2 topic echo /wall_tracker/pose --once
ros2 topic hz /wall_tracker/pose
```
